...
function tutoriallink(tutorial, className) {
    const raw = helper.toTutorial(tutorial, null, {
        tag: 'em',
        classname: className || 'disabled',
        prefix: 'Tutorial: '
    });

    // this allows us to add custom classes to tutorial links
...
function linktoTutorial(longName, name, className) {
    return tutoriallink(name, className);
}
...
exports.publish = function(taffyData, opts, tutorials) {
    ...
    staticDir = path.join(templatePath, 'static');
    staticFiles = fs.ls(staticDir, 3);

    staticFiles.forEach(function(fileName) {
        var toDir = fs.toDir( fileName.replace(staticDir, path.join( outdir, 'static' ) ));

        fs.mkPath(toDir);
        fs.copyFileSync(fileName, toDir);
    });

    // copy the template's style files to outdir
    styleDir = path.join(templatePath, 'styles');
    staticFiles = fs.ls(styleDir, 3);
